...
- **`niagara insert_dynamic_input` — registered, and DISABLED.** The action exists, is discoverable, and always refuses, naming the outside-in workaround (`get_dynamic_input_tree` → `remove_dynamic_input` → `add_dynamic_input`). It is disabled because the move-based implementation it shipped with **stack-overflowed the editor on its first invocation**, on the simplest topology there is. See *Fixed* below for the root cause and for the replacement, which is written but has never been run.

- **A cycle guard on every override-chain mutation.** `add_dynamic_input` and `insert_dynamic_input` now take a cycle reading of the graph *before* the transaction opens and again before returning control to the engine. A graph that arrives already cyclic is refused outright — it is a crasher in its own right, and no action should be the thing that finally walks it. A cycle that appears only afterwards is provably one we created, which is what licenses the guard to cut the back edge rather than merely report it: a disconnected graph is a compile error the caller can see, a cyclic one is a dead editor. Detection is an **iterative** three-colour DFS over every pin link, not just ParameterMap links — the crash cycle ran a value-output edge one way and a map edge the other, and a recursive detector would have overflowed on exactly the graphs it exists to catch. The guard deliberately does not lean on `GEditor->CancelTransaction` for rollback: `UTransBuffer::Cancel` pops the transaction off the undo buffer and calls `FTransaction::EndOperation`, never `FTransaction::Apply`, so it discards the undo *record* rather than reverting the objects.

### Fixed

- **`insert_dynamic_input` killed the editor with `EXCEPTION_STACK_OVERFLOW`.** The implementation moved the existing chain: break the module's override pin, add the new dynamic input, re-point links so the old chain hangs off `into_input`. On a depth-1 `SpawnRate → Multiply_Float(A=2, B=3)`, inserting `Add_Float` into `A`, the editor died with ~8250 stack frames of which every frame after the first two was the same return address inside `UnrealEditor-NiagaraEditor.dll` — one function recursing on itself until the stack was gone, and no Monolith frames anywhere: the graph was mutated into a cycle and an engine map walk followed it. The cause was not `GetOrCreateStackFunctionOverrideNode`'s link theft, which an earlier repair had targeted. It was `GetStackFunctionOverrideNode`, which answers "what is this function call's override node?" with "whatever `UNiagaraNodeParameterMapSet` my map input happens to be linked to". After the new node is placed, `SetDynamicInputForFunctionInput` has linked its map input to the *captured chain's own* override MapSet, so the new node adopts that MapSet as its own, the `into_input` override pin is created **on it**, and re-attaching the captured chain's value output to that pin wires a node into the very MapSet it reads its overrides from. Sharing an override node that way is normal and harmless when the engine builds the graph — it always attaches a value node to the override node's *upstream* pin — and is only fatal when an existing, already-downstream node is wired back into it. Which is what moving a chain means. The action now refuses; the crash is unreachable.

- **`insert_dynamic_input` no longer does graph surgery (written, unvalidated, behind an opt-in).** The replacement stops moving live nodes and does what the editor's cut → add → paste actually does: capture the chain as data, remove it with `remove_dynamic_input`'s proven recursive teardown, add the new dynamic input through the ordinary path, then recreate the captured chain under `into_input` using only `GetOrCreateStackFunctionInputOverridePin` + `SetDynamicInputForFunctionInput`. Every link is made by the engine in the order the engine makes them, so the result is the topology a human building the same chain outside-in would get, and the cycle class is structurally unreachable. Capture **refuses rather than guesses**, because a rebuild that silently drops data would only trade a crash for data loss: custom HLSL dynamic inputs (the body is not in `FunctionScript`), parameter bindings, data interfaces and object assets (only the parameter *name* is visible to the traversal, not the object), propagated static switch parameters, any static switch whose selection differs from `AutogeneratedDefaultValue` (a rebuilt node silently reverts, and a switch decides which inputs exist), and any node carrying input pins the stack enumeration does not report. Literals are replayed as the pin's `DefaultValue` **string verbatim**, never re-encoded, so no type can be lost to a spelling mismatch. Every failure after the removal rebuilds the original chain in place from the same capture, and every post-mutation error carries the captured tree as JSON. Node GUIDs change (invariant I-19). **This code has not been compiled or run** — it is reachable only via an `allow_unvalidated_rebuild` flag intended for validation, which should be deleted once the rewrite has passed.

- **`add_dynamic_input` accepted a script that is not a dynamic input, and produced an unremovable node.** Nothing checked `UNiagaraScript::GetUsage()`, so a **module** script attached happily, compiled with no new error, and then could not be removed by GUID at all — `remove_dynamic_input` reports "Could not find the override pin connected to this dynamic input", because a module script never gets the single value output pin the removal path traces back. Anything that is not `ENiagaraScriptUsage::DynamicInput` is now refused before any mutation, naming the actual usage in engine spelling and pointing module scripts at `add_module`.
- **`add_dynamic_input` accepted a type-mismatched script and let the VM truncate it.** A Vector3f dynamic input on a float input was attached silently and compiled with only `implicit truncation from 'vec3' to 'float'` buried in the translator warnings — the value was quietly reduced to `.x`. The script's output type is now resolved exactly the way the editor's own picker resolves it (single `UNiagaraNodeOutput`, single value input pin) and checked with `FNiagaraEditorUtilities::AreTypesAssignable`, the same predicate that decides which scripts the picker offers. Mismatches are refused with both types named. Responses gain `output_type` and `target_input_type`.
- **Replacing a dynamic input chain leaked graph nodes that no audit could see.** Breaking the override pin's link detaches only the displaced chain's *root*: every node below it kept its ParameterMap links, stayed inside the engine's compiled traversal, and went on computing into override parameters nothing reads. Node count grew on every replace while `audit_stack_wiring` reported `orphan_count: 0` and `clean_stack_orphans` found nothing. The editor never had this because `UNiagaraStackFunctionInput::SetDynamicInput` calls `RemoveNodesForOverridePin` first; its engine implementation is not exported, so the replace now reuses `remove_dynamic_input`'s own recursive teardown, which was already correct. Behaviour is otherwise unchanged — the replace still replaces. The response now reports what it displaced as `displaced_chain` (kind, GUID, script path, the full input tree, and `nodes_removed`) plus a `replaced` flag, so a caller cannot silently lose work. The teardown additionally removes a displaced `UNiagaraNodeParameterMapGet`, matching the engine, so replacing a parameter binding no longer strands its Get node.
- **`audit_stack_wiring` can see dead-but-traversed dynamic input nodes.** Its orphan test only caught fully disconnected nodes and Get nodes feeding nothing, which is precisely what a leaked chain node is not. A new `dead_dynamic_input` orphan kind flags any non-module `UNiagaraNodeFunctionCall` whose value output pin is linked to nothing while its map pins keep it in the traversal, with the GUID to hand to `remove_dynamic_input`. Detection only: sweeping these in `clean_stack_orphans` would also have to splice each node's override MapSet out of the map chain, which is `remove_dynamic_input`'s job.
- **`set_module_input_value` / `set_dynamic_input_value` validate scalar literals.** `"not_a_number"` on a `NiagaraInt32` and `"hello"` on a `NiagaraFloat` were both stored verbatim, read back verbatim, and compiled with 0 errors and 0 warnings — a wrong runtime value with no signal anywhere. Bool, int32 and float literals are now checked against the pin's real type before the transaction opens, in the same style as `set_static_switch_value`; bool spellings normalise (`yes` → `true`) and an integer written as `5.000000` normalises to `5`. Scope is deliberately limited to those three types, whose pin encoding is verifiable in engine source; every other type still passes through untouched.
- **`set_module_input_value` wrote the wrong pin encoding for `Vector2f` and `LinearColor`, silently.** The literal was built with one generic comma join for every composite type. That happens to be the engine's pin encoding for `Vector3f`, `NiagaraPosition`, `Vector4f` and `Quat4f`, and is *not* the encoding for `Vector2f` (`(X=…, Y=…)`) or `LinearColor` (`(R=…,G=…,B=…,A=…)`). Measured in compiled GPU HLSL, `Vector2f = "12.5,34.25"` became `float2(0,0)` and `LinearColor = "0.9375,0.8125,0.6875,0.5625"` became `float4(0,0,0,1)` — 0 errors, 0 warnings, and both read back verbatim, so nothing surfaced it. It is not a fallback to the declared default either: that colour's default is `(1,1,1,1)`. The engine cannot report this itself because `UEdGraphSchema_Niagara::PinToNiagaraVariable` parses the pin into an *unallocated* `FNiagaraVariable`, and every composite `SetValueFromPinDefaultString` ends in `|| !Variable.IsDataAllocated()` — on an unallocated variable that clause commits the half-parsed value and returns `true`, so the schema's own "failed to convert" warning never fires. Composite literals are now validated and, if necessary, re-spelled through the type's **own registered `INiagaraEditorTypeUtilities`** — the same registry the compiler resolves through — by probing candidate spellings against an **allocated** variable, which is what makes that parser able to fail at all. Being registry-driven, this is correct for types nobody has exercised, and types with no registered pin-default handling (`NiagaraMatrix4`, data interfaces, user structs) are left exactly as before. Both spellings are accepted on input for every composite type and the caller's own spelling is always tried first, so a value the engine already accepted is written byte-for-byte and loses no precision; only a spelling the type genuinely rejects is re-labelled, and the response says so. Numbers are never reformatted — re-emitting through `GetPinDefaultStringFromValue` would have been lossy, since the vector encoders are `%3.3f` and would silently zero a legitimate `0.0001`. Non-scalar inputs also stop accepting garbage: `Vector2f = "hello"` and `LinearColor = "banana"` were both stored without complaint before, as was a component count that did not match the type. The JSON-object form (`{"x":…,"y":…}`) normalises into the same path and is fixed by the same change.
- **`search_dynamic_inputs` found dynamic inputs by folder name.** Every candidate whose package path did not contain `/DynamicInputs/` was skipped, so a dynamic input stored anywhere else — including anything authored in a project — returned zero hits while working perfectly when passed by direct path. Discovery now asks the question the editor's own picker asks: `FNiagaraEditorUtilities::GetFilteredScriptAssets` filtered on `ENiagaraScriptUsage::DynamicInput`, with non-library scripts included. `inferred_output_type` was also guessed from the asset name (it reported `SetBoolByFloatComparison` as `float`); the real output type is now read off the script's output node and reported as `output_type`, with `inferred_output_type` retained as an alias.
- **`get_dynamic_input_inputs` returned the wrong inputs.** It enumerated `UNiagaraNodeInput` nodes only, which for a typical dynamic input is a single node — the graph's ParameterMap input — so `Multiply_Float` reported one input named `NewInput` of type `NiagaraParameterMap` instead of `A` and `B`. A dynamic input's inputs are `Module.*` parameters read out of the parameter map, not input nodes. They are now collected from the graph's script-variable registry and from the `Module.*` output pins of its ParameterMap Get nodes, unioned and deduped, with static switches enumerated through the same shared helper `get_module_script_inputs` uses. The response also reports the script's `usage`, `is_dynamic_input` and real `output_type`.
