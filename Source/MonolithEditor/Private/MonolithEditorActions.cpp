...
void FMonolithEditorActions::RegisterActions(FMonolithLogCapture* LogCapture)
{
	...
	Registry.RegisterAction(TEXT("editor"), TEXT("set_preview_profile"),
		TEXT("SWITCH the ACTIVE asset-viewer preview-scene profile by name — the write half of get_preview_scene's active_profile_* fields (set_preview_scene mutates a profile's VALUES; this changes WHICH profile is active). Drives FPreviewProfileController::SetActiveProfile, the same bridge the editor's own preview-profile dropdown uses: it writes UEditorPerProjectUserSettings::AssetViewerProfileIndex (UPROPERTY(Config)-only, unreachable from Python) and broadcasts the settings-changed event, so ALREADY-OPEN asset editors restyle with no reopen. Returns a THREE-WAY VERDICT instead of the engine's boolean, because that boolean is FALSE both for 'no such profile' AND for 'already active' (PreviewProfileController.cpp:81): result='switched' (verified by RE-READING the active index after the call — the raw bool is reported as engine_return_value for the record only), result='already_active' (success; no engine call is made, since it could only return the ambiguous false), or a not_found ERROR listing the available names (profiles are never created). The response's active_profile_* / profiles[] are the post-call re-read, never an echo. SCOPE: this changes the GLOBAL profile index — Niagara, Material, Static Mesh, PCG and Persona honour it, but Dataflow and Chaos Cloth track their profile BY NAME and ignore the global index, so it is not 'all editors'. PERSISTENCE: the selection lives in the per-user Saved/Config/<Platform>/EditorPerProjectUserSettings.ini (typically untracked) via the editor's normal user-settings save — this action itself writes no file, exactly like the engine's own dropdown. Etiquette: the selection outlives the session, so read the current profile first (previous_profile_name is returned to make this easy) and switch back when done."),
		FMonolithActionHandler::CreateStatic(&HandleSetPreviewProfile),
		FParamSchemaBuilder()
			.Required(TEXT("name"), TEXT("string"), TEXT("Profile name to activate, matched CASE-INSENSITIVELY against FPreviewSceneProfile::ProfileName, first match wins — mirroring the engine's own IndexOfByKey lookup. Read the available names from editor::get_preview_scene (e.g. \"Grey Ambient\", \"Epic Headquarters\", \"Grey Wireframe\"). An unknown name is an error listing them."))
			.Build());

	// --- Editor viewport client enumeration ---
	// Body lives in MonolithEditorViewportCameraActions.cpp.

	Registry.RegisterAction(TEXT("editor"), TEXT("list_viewports"),
		TEXT("READ-ONLY enumeration of EVERY live editor viewport client (GEditor->GetAllViewportClients()) — including the per-asset-editor PREVIEW viewports (Niagara, Material, Static Mesh, PCG, Persona, ...) that no Python or native-toolset route can reach, because FEditorViewportClient is not a UObject and exposes no UFUNCTION. Per viewport: index, kind ('preview' when the client carries an FPreviewScene, else 'level'), widget_type (the Slate type name, e.g. 'SNiagaraSystemViewport' — the cast-free identification key), world_path, preview_assets (Niagara systems found in that preview world — identifies the viewport for a NAMED asset, not merely 'a Niagara viewport'), viewport_type, is_perspective, is_realtime, is_visible, has_widget/has_world/has_viewport, size, and the full camera state. THE LOAD-BEARING FIELD IS effective_camera_location: preview viewports are in ORBIT MODE BY DEFAULT, and in orbit mode the rendered camera is computed from look_at + yaw/pitch + the DISTANCE of view_location from look_at — the DIRECTION of view_location and the ROLL are discarded — while GetViewLocation() still returns verbatim whatever was written. So a naive camera read-back round-trips perfectly over a camera that renders from somewhere else. effective_camera_location / effective_camera_rotation are RECOMPUTED from the orbit matrix (the same inversion the engine itself uses), and view_location_is_effective is false exactly when the stored location is not where the view actually renders from. FOV is likewise reported TWICE — view_fov (current) and fov_angle (stored/ini) — with fov_diverged set when they disagree, because Epic's own setter writes only one of them and several engine paths restore one from the other. Use this to find the right viewport before driving a camera, and to verify a camera actually took effect. Mutates nothing."),
		FMonolithActionHandler::CreateStatic(&HandleListViewports),
		MakeShared<FJsonObject>());

	Registry.RegisterAction(TEXT("editor"), TEXT("set_viewport_camera"),
		TEXT("MOVE an editor viewport camera — including the PREVIEW viewport inside an open Niagara / Material / Static Mesh / PCG / Persona editor, which no Python or native-toolset route can reach. The primary interface is WHAT TO LOOK AT (look_at), HOW FAR AWAY (distance) and FROM WHAT ANGLE (angle preset, or raw yaw/pitch) — not an absolute position, and that is deliberate. In ORBIT MODE, WHICH IS THE DEFAULT FOR PREVIEW VIEWPORTS, SetViewLocation() CANNOT PLACE THE CAMERA: only the distance |view_location - look_at| survives into the rendered view, the direction and the roll are discarded, yet the value reads back verbatim — so passing `location` to an orbiting viewport is REFUSED with an error naming look_at/distance/angle, never silently accepted. This action also never toggles orbit mode behind your back, because ToggleOrbitCamera rewrites ViewRotation and jumps the camera. ANGLE PRESETS: top | bottom | front | back | left | right | persp45. Each resolves to a yaw/pitch pair derived from the engine's own orbit matrix, and the axis presets reproduce the editor's own ortho Top/Bottom/Front/Back/Left/Right framing exactly, in-plane roll included. THE PRESET'S YAW DIFFERS BETWEEN ORBIT AND NON-ORBIT MODE (the two modes parameterise azimuth with opposite sign: Y_orbit = 90 - Y_nonorbit) — the action picks the right one from the viewport's actual mode, which is the main reason to use a preset over a raw yaw. `front` puts the camera at +X looking toward -X; `right` at +Y; `top` at +Z; `persp45` is a three-quarter view halfway between front and right, looking down 45 degrees. Raw `yaw`/`pitch` are written verbatim and override a preset's components; `rotation` [pitch,yaw,roll] is a full explicit rotator and is mutually exclusive with angle/yaw/pitch. `look_at` defaults to the viewport's CURRENT pivot (which moves when a user pans) — pass reset_look_at:true to put it back at the origin. `distance` defaults to the current radius, falling back to 256 if that is degenerate. `fov` writes BOTH ViewFOV and FOVAngle, because Epic's own setter writes only one and several engine paths restore it from the other. Invalidates the viewport it touched by default (the engine's own camera setters do not invalidate at all, and the engine's invalidate-all helper redraws the ACTIVE viewport regardless of which one you moved). ZERO MUTATING PARAMS IS AN ERROR, never a silent no-op. Returns the SAME per-viewport shape editor::list_viewports returns, RE-READ AFTER THE WRITE — so `effective_camera_location` in the response is the recomputed rendered position, not an echo of what was sent. ⚠️ `target` accepts \"index:N\" (POSITIONAL in GEditor->GetAllViewportClients() and RENUMBERS whenever any editor window closes — re-read editor::list_viewports immediately before targeting), \"widget:SNiagaraSystemViewport\", \"asset:/Game/Path/NS_Foo\" (matched against the client's preview world), or \"level\" (the one perspective level viewport). An ambiguous target is an error listing the candidates, never a silent pick."),
		FMonolithActionHandler::CreateStatic(&HandleSetViewportCamera),
		FParamSchemaBuilder()
			.Required(TEXT("target"), TEXT("string"), TEXT("Which viewport to drive: \"index:N\" (from editor::list_viewports — POSITIONAL, and it RENUMBERS when any editor closes, so re-read before targeting), \"widget:SNiagaraSystemViewport\" (the widget_type key; exact match preferred, substring only if nothing matches exactly), \"asset:/Game/Path/NS_Foo\" (an asset in the client's preview world; the .AssetName suffix is optional), or \"level\" (the single PERSPECTIVE level viewport). Ambiguous matches are refused with the candidate list."))
			.Optional(TEXT("angle"), TEXT("string"), TEXT("Named view: top | bottom | front | back | left | right | persp45. Sets yaw AND pitch. front=camera at +X, back=-X, right=+Y, left=-Y, top=+Z, bottom=-Z, persp45=+X+Y looking down 45 degrees. The yaw used depends on whether the viewport is orbiting — that is handled for you. Mutually exclusive with 'rotation'; individual 'yaw'/'pitch' override the preset's component."))
			.Optional(TEXT("yaw"), TEXT("number"), TEXT("Raw ViewRotation.Yaw in degrees, written VERBATIM (no mode correction). Overrides an 'angle' preset's yaw. Mutually exclusive with 'rotation'."))
			.Optional(TEXT("pitch"), TEXT("number"), TEXT("Raw ViewRotation.Pitch in degrees, written VERBATIM. Negative pitch looks DOWN. Overrides an 'angle' preset's pitch. Mutually exclusive with 'rotation'."))
			.Optional(TEXT("rotation"), TEXT("array"), TEXT("[pitch, yaw, roll] full rotator. Mutually exclusive with angle/yaw/pitch. ROLL IS DISCARDED while orbiting — the orbit matrix reads only yaw and pitch."))
			.Optional(TEXT("look_at"), TEXT("array"), TEXT("[x,y,z] orbit pivot — the point the camera looks at. Defaults to the viewport's CURRENT pivot, which moves when the user pans. Mutually exclusive with 'location'."))
			.Optional(TEXT("reset_look_at"), TEXT("bool"), TEXT("Put the orbit pivot back at the world origin [0,0,0]. Equivalent to look_at:[0,0,0]; exists because panning moves the pivot and there is otherwise no obvious way to get it home. Default false."), TEXT("false"))
			.Optional(TEXT("distance"), TEXT("number"), TEXT("Camera distance from look_at, in world units. Must be > 0. Defaults to the viewport's current radius; if that is degenerate (< 1) it falls back to the engine default of 256 and says so in warnings[]. Mutually exclusive with 'location'."))
			.Optional(TEXT("location"), TEXT("array"), TEXT("[x,y,z] ABSOLUTE camera position. ACCEPTED ONLY WHEN THE VIEWPORT IS NOT ORBITING — on an orbiting viewport it is REFUSED, because SetViewLocation cannot place an orbit camera and the write would read back perfectly while moving the camera only radially. Mutually exclusive with look_at/distance/reset_look_at."))
			.Optional(TEXT("fov"), TEXT("number"), TEXT("Horizontal field of view in degrees, 5..170. Writes BOTH ViewFOV (current) and FOVAngle (stored) — Epic's own setter writes only ViewFOV, and several engine paths then restore it from FOVAngle. Out-of-range values are refused, not clamped."))
			.Optional(TEXT("orbit_mode"), TEXT("bool"), TEXT("Explicitly turn the orbit camera on/off via ToggleOrbitCamera. USE SPARINGLY: the toggle REWRITES ViewRotation from the orbit matrix, so the camera visibly jumps. Applied BEFORE everything else, and any 'angle' preset then resolves against the resulting mode. Omitted = leave the mode alone."))
			.Optional(TEXT("realtime"), TEXT("bool"), TEXT("FEditorViewportClient::SetRealtime. NOTE: the engine PERSISTS this between editor sessions — it is not a temporary override."))
			.Optional(TEXT("invalidate"), TEXT("bool"), TEXT("Invalidate the viewport we actually touched so a non-realtime viewport redraws. Default true — the engine's own camera setters do not invalidate, and its invalidate-all helper ignores which viewport you moved."), TEXT("true"))
			.Build());
